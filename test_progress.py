...
运行：`$env:PYTHONIOENCODING='utf-8'; & "E:\\anaconda\\envs\\python_class\\python.exe" test_progress.py`
"""
import asyncio
import os
import shutil
...
def build_engine():
    ...
    world = World()
    bus = EventBus()
    db_dir = tempfile.mkdtemp(prefix="ktown_progress_")
    storage = Storage(os.path.join(db_dir, "test.db"))
    ...
    return engine, agents, storage, db_dir


async def main():
    print("=== 1. 身世之谜：思考得线索 → 调查收集碎片（游戏化流程） ===\n")
    engine, agents, storage, db_dir = build_engine()
    try:
