...
def create_app(world,agents,bus,logger,knowledge,llm):
    ...
    async def get_timeline(day:int=0):
        events = logger.query_world_events(100)
        if day>0:
            base=(day-1)*24
    ...
    async def ws_endpoint(websocket:WebSocket):
        ...
        try:
            await websocket.send_json({"type":"state","data":{"tick":world.state.tick,"weather":world.state.weather,"agents":[a.to_dict() for a in agents]}})
            while True:
