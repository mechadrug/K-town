...
## 下一步（按 v0.4 计划 + 设计主计划）
- **玩法设计**：`docs/product/gameplay-design-v3.md`（权威玩法，取代旧玩家英雄稿）
- **v0.4 功能/实现安排**：`docs/plans/2026-08-04-gameplay-plan.md`
  - P0 修复四类前端问题：F0.2 Tab显示 → F0.1 图标系统 → F0.4 交互模型 → F0.5 小镇脉搏首屏 → F0.3 对话重设计
  - P0 功能：F0.6 每日目标薄层 → F0.7 知识流动视图
  - P1 深化：Agent日记 → 关系后果化 → 食物经济闭环 → 事件链
  - P2 后置：成就/时代/叙事取舍、知识持久化、tick.py 拆分
- **长期**（设计主计划 Phase 1–4）：30 天稳定性、tick<3s、知识持久化、VACUUM/外键/索引
