...
当前产品基线是四周 Demo。章节内容由 `campaign.py` 导演层管理；前端状态契约、API 客户端和事件委托边界见 `frontend-v2.md`。
...
### 2.13 章节导演（campaign.py）
- 按游戏日解锁四周篇章并注册请求
- 解释结构化请求效果，写入章节旗标、评分、世界标记、知识和关系
- 保存章节历史并在第 29 天生成最终结局
...
5. 知识模块处理知识变化
6. 日志模块记录所有变化
7. `CampaignDirector` 把章节内容效果写入统一 ActionResult 和世界标记
8. API 模块通过同一状态构建器向 REST/WebSocket 提供状态
9. 前端契约归一化状态，再由视图和事件路由更新界面
10. 数据库模块定期保存快照
