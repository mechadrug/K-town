...
├── events.py              # 事件系统（事件总线、调度器、12种事件类型）
├── knowledge.py           # 知识引擎（创建、传播、质疑、修正、固化、搜索）
├── tick.py                # Tick循环引擎（模拟循环、每日摘要、数据持久化）
├── api.py                 # REST API和WebSocket接口
├── config.py              # 配置加载
├── config.yaml            # 配置文件
├── db.py                  # 数据库持久化（SQLite）
├── logger.py              # 日志系统（世界事件、Agent决策、知识变化、玩家操作）
├── templates/
│   └── index.html         # 前端控制台
├── docs/
...
## 玩家操作说明
1. **移动**：点击操作面板上的"移动到XXX"按钮，可以移动到任意地点
2. **添加知识**：点击"添加知识"按钮，填写主题和内容，可以创建新知识
3. **发起交易**：点击"发起交易"按钮，选择交易对象、物品和价格，发起交易请求
4. **接受交易**：在交易列表里，点击"接受"按钮，可以接受其他Agent发起的交易
5. **重置模拟**：点击状态栏的"重置模拟"按钮，可以清空所有数据，重新开始模拟
